...
class RdsMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: ASGIApp, application: Application, async_db: AsyncIOMotorDatabase) -> None:
        super().__init__(app)
        self.__rds_config = application.rds_config
        self.__rds_key = application.setting.rds_key
        self.__service = AsyncRdsServiceImp(AsyncRdsMapperImp(async_db, application.setting))
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        request.state.service = self.__service
